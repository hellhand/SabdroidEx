...
public class ComingListRowAdapter extends ArrayAdapter<Object[]> implements SectionIndexer {

    private final Context mContext;
    private final LayoutInflater mInflater;
    private final ArrayList<Object[]> rows;
    private final HashMap<String, Integer> periods;
    ...
    public ComingListRowAdapter(Context context, ArrayList<Object[]> rows) {
        super(context, R.layout.coming_item);
        ...
        this.periods = new LinkedHashMap<String, Integer>();
        for (int i = 0; i < rows.size() ;i++) {
        	Object[] element = rows.get(i);
        	if (element.length == 2)
        	{
        		if ((Integer) element[1] != 0) {
					periods.put((String) element[0], (Integer) element[1]);
        		}
        	}
        	else {
	...
	public Object[] getSections() {
		return periods.keySet().toArray();
	}

	@Override
	public int getPositionForSection(int section) {
		int elementCount = 0;
		Object[] keys = periods.keySet().toArray();
		for (int i = 0; i < section ;i++) {
			elementCount += periods.get(keys[i]);
		}
		return elementCount;
	}
