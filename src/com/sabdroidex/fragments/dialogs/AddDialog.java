...
public class AddDialog extends DialogFragment {
	...
	public static void setAddDialogListener(
			DialogActionsListener addDialogListener) {
	...
	public Dialog onCreateDialog(Bundle savedInstanceState) {
		...
		if (Preferences.isEnabled(Preferences.SICKBEARD)) {
			options.add(getResources()
					.getString(R.string.add_show_dialog_title));
		}
		if (Preferences.isEnabled(Preferences.COUCHPOTATO)) {
			options.add(getResources().getString(
					R.string.add_movie_dialog_title));
		}

		builder.setItems(options.toArray(new CharSequence[options.size()]),
				new OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						if (which == 0) {
							...
							addDialogListener.showAddNzbDialog();
								...
								&& Preferences.isEnabled(Preferences.SICKBEARD)) {
							dialog.dismiss();
							addDialogListener.showAddShowDialog();
						} else if (which >= 1
								&& Preferences
										.isEnabled(Preferences.COUCHPOTATO)) {
							dialog.dismiss();
